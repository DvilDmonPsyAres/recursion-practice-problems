...
function makeBetterChange(target, coins = [25, 10, 5, 1], memo = {}) {
  // Check if the result for the current target amount is already memoized
  if (memo[target]) {
    return memo[target];
  }

  // Base case: if the amount is 0, return an empty array to indicate a successful combination
  if (target === 0) {
    return [];
  }

  // Base case: if the amount is negative or there are no coins available, return null to indicate no valid combination
  ...
  // Try using each coin and recursively find a combination for the remaining amount
  for (let i = 0; i < coins.length; i++) {
    const coin = coins[i];
    const remainingCoins = coins.slice(i); // Exclude the used coin from the remaining coins
    const remainingTarget = target - coin; // Calculate the remaining target amount

    const change = makeBetterChange(remainingTarget, remainingCoins, memo); // Recursive call

    // If a valid combination is found, add the current coin to the beginning of the combination and store it in the results
    if (change !== null) {
      results.push([coin, ...change]);
    }
  }

  // If no valid combination is found, memoize null and return null
  if (results.length === 0) {
    memo[target] = null;
    return null;
  }

  // Find the combination with the fewest number of coins from the results
  const bestChange = results.reduce((min, change) => {
    if (change.length < min.length) {
      return change;
    }
    return min;
  });

  // Memoize the best combination and return it
  memo[target] = bestChange;
  return bestChange;
}
